...
class EventStream(object):
    def __init__(self, event_handler, heartbeat_handler, args):
        ...
        self.event_stream = sseclient.SSEClient('https://arlo.netgear.com/hmsweb/client/subscribe?token='+self.arlo.request.session.headers.get('Authorization'), session=self.arlo.request.session)
        self.thread = threading.Thread(name="EventStream", target=event_handler, args=(args[0], self.event_stream, ))
...
class Arlo(object):
    ...
    def Unsubscribe(self, basestation):
        basestation_id = basestation.get('deviceId')
        if basestation_id in self.event_streams:
            print("found stream in unsubscribe")
            if self.event_streams[basestation_id].connected:
                print("stream was connected")
    ...
    def HandleEvents(self, basestation, callback, timeout=None):
        ...
        basestation_id = basestation.get('deviceId')
        if DEBUG:
            print("basestation_id: {}".format(basestation_id))

        self.Subscribe(basestation)
        if DEBUG:
            print("event stream for basestation:")
            pprint(vars(self.event_streams[basestation_id]))
        if basestation_id in self.event_streams and self.event_streams[basestation_id].connected and self.event_streams[basestation_id].registered:
    ...
    def TriggerFullFrameSnapshot(self, basestation, camera):
        def callback(self, basestation, event):
            if DEBUG:
                print("TriggerFullFrameSnapshot event: {}".format(event))
